...
class Vocab:
    ...
    def __init__(self, filename=None, initial_tokens=None, lower=False):
        ...
        # padding，把格式补整齐，在训练中把他的embedding设为零
        self.unk_token = '<unk>'
        # unknown
        # 只设一个unknown token，而且在训练中把他的embedding设为零
    ...
    def get_id(self, token):
        """
        gets the id of a token, returns the id of unk token if token is
        not in vocab
        Args:
    ...
    def convert_to_ids(self, tokens):
        """
        Convert a list of tokens to ids, use unk_token if the token is not
        in vocab.
        Args:
    ...
    def recover_from_ids(self, ids, stop_id=None):
        """
        Convert a list of ids to tokens, stop converting if the stop_id is
        encountered
        Args:
