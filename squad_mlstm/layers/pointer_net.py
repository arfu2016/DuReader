...
@Created   : 7/26/18 10:48 AM
@Desc      :
This module implements the Pointer Network for selecting answer spans,
as described in: https://openreview.net/pdf?id=B1-q5Pqxl
"""
...
def custom_dynamic_rnn(cell, inputs, inputs_len, initial_state=None):
    ...
    function in Tensorflow seem to require the hidden unit and memory unit has
    the same dimension, and we cannot store the scores directly in the
    hidden unit.
    Args:
        cell: RNN cell
    ...
    def loop_fn(t, prev_s, emit_ta, finished):
        ...
        if isinstance(cell, tc.rnn.LSTMCell):
            ...
            cur_state = tc.rnn.LSTMStateTuple(
                tf.where(finished, prev_c, cur_c),
    ...
    with tf.variable_scope(scope or 'attend_pooling'):
        U = tf.tanh(
            tc.layers.fully_connected(
                pooling_vectors, num_outputs=hidden_size,
                activation_fn=None,
                biases_initializer=None) + tc.layers.fully_connected(
                tf.expand_dims(ref_vector, 1),
        ...
        with tf.variable_scope(scope or type(self).__name__):
            U = tf.tanh(
                self.fc_context + tf.expand_dims(
                    tc.layers.fully_connected(
                        m_prev, num_outputs=self._num_units,
...
class PointerNetDecoder(object):
    ...
    def decode(self, passage_vectors, question_vectors,
               init_with_question=True):
        """
        ...
        Args:
            ...
            init_with_question: if set to be true,
                             we will use the question_vectors to init the
                             state of Pointer Network
        Returns:
        ...
        with tf.variable_scope('pn_decoder'):
            ...
            # not used, acutally used but useless
            sequence_len = tf.tile([2], [tf.shape(passage_vectors)[0]])
            if init_with_question:
                random_attn_vector = tf.Variable(
                    tf.random_normal([1, self.hidden_size]),
                ...
                # 一个表示cell state，另一个表示hidden state
                # 这里都用pooled_question_rep来初始化
            else:
                init_state = None
